...
/**
   * loadContent
   * @returns {Promise<void>}
   */const loadContent=async content_container=>{Logger.debug(`Loading content...`,w.racing_plus);if(content_container.querySelector("#racingupdates")){await PageContent.loadOfficialEvents()}else if(content_container.querySelector(".pm-categories")){await PageContent.loadModifications()}else if(content_container.querySelector("#racing-leaderboard-root")){await PageContent.loadStatistics()}else if(content_container.querySelector(".enlist-wrap")){const title=content_container.querySelector(".enlisted-btn-wrap");if(title?.innerText.toLowerCase().includes("official race")){await PageContent.loadOfficialEvents()}else if(title?.innerText.toLowerCase().includes("custom race")){await PageContent.loadCustomEvents()}else if(content_container.querySelector(".info-msg")){await PageContent.loadModifications()}else{await PageContent.loadCars()}}Logger.info(`Content loaded.`,w.racing_plus)};
/**
