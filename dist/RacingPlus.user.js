...
// ...
// build query string
let queryString="";if(params!=null&&typeof params==="object"&&Object.entries(params).length>0){queryString=Object.entries(params).map(([k,v])=>`${encodeURIComponent(k)}=${encodeURIComponent(v)}`).join("&")}else{throw new Error("Invalid argument. Params must be an object.")}
// build query url
const queryURL="https://api.torn.com/v2"+`/${root}/${path.replace(/^\/+|\/+$/g,"")}`+`?comment=${API_COMMENT}${this.key?`&key=${this.key}`:""}${queryString?`&${queryString}`:""}`;
// check for cached copy, then return results
const cached=this.cache.get(queryURL);if(cached&&Date.now()-cached.timestamp<CACHE_TTL)return cached.data;
// no cached copy, request new copy
const controller=new AbortController;const timer=setTimeout(()=>controller.abort(),API_FETCH_TIMEOUT);try{
// get response
const response=await fetch(queryURL,{signal:controller.signal});if(!response.ok){throw new Error(`HTTP ${response.status} ${response.statusText||""}`.trim())}
// parse response
const results=await response.json().catch(err=>{throw new Error(`Invalid JSON response: ${err}`)});if(!results||results.error){throw new Error(`API request failed: ${results?.error?.error??"Unknown error."}`)}
...
const driverId=(drvr.id||"").substring(4);const driverStatus=drvr.querySelector(".status");const drvrName=drvr.querySelector("li.name");const nameLink=drvrName?.querySelector("a");const nameSpan=drvrName?.querySelector("span");const drvrColour=drvr.querySelector("li.color");
// Update status icon classes
if(driverStatus){switch(this.status){case"joined":driverStatus.className="status success";driverStatus.textContent="";break;case"waiting":driverStatus.className="status waiting";driverStatus.textContent="";break;case"racing":driverStatus.className="status racing";driverStatus.textContent="";break;case"finished":default:break}}
// Fix driver colours
if(drvrColour&&nameSpan){drvrColour.classList.remove("color");nameSpan.className=drvrColour.className}
// Add driver profile links
if(addLinks){if(!nameLink&&nameSpan){nameSpan.outerHTML=`<a target="_blank" href="/profiles.php?XID=${driverId}">${nameSpan.outerHTML}</a>`}}else{if(nameLink){drvrName.innerHTML=`${nameLink.innerHTML}`}}
// Fix driver race stats
if(!drvr.querySelector(".statistics")){
// Add stats container
drvrName.insertAdjacentHTML("beforeEnd",`<div class="statistics"></div>`)}const stats=drvr.querySelector(".statistics");
// Adjust time
const timeLi=drvr.querySelector("li.time");if(timeLi){if(timeLi.textContent===""){timeLi.textContent="0.00 %"}const timeContainer=w.document.createElement("ul");timeContainer.appendChild(timeLi);stats.insertAdjacentElement("afterEnd",timeContainer)}
// Show driver speed
if(showSpeed){if(!stats.querySelector(".speed")){stats.insertAdjacentHTML("beforeEnd",'<div class="speed">0.00mph</div>')}
...
// Show driver skill
if(showSkill){if(!stats.querySelector(".skill")){stats.insertAdjacentHTML("afterBegin",'<div class="skill">RS: ?</div>')}if(torn_api.key){
// Fetch racing skill data from the Torn API for the given driver id
try{let user=await torn_api.request(`user/${driverId}/personalStats`,{stat:"racingskill"});if(user){let skill=stats.querySelector(".skill");skill.textContent=`RS: ${user.personalstats?.racing?.skill??"?"}`}}catch(err){console.log(`[TornPDA.Racing+]: ${err.error??err}`)}}}}//);
}}
/**
...
/**
   ...
   * @returns {Promise<void>}
   */const initRacingPlusPanel=async()=>{Logger.debug("Initializing settings panel...");
...
/**
   ...
   * @param {} tabs
   * @returns {Promise<void>}
   */const fixActiveTabHighlighting=async()=>{Logger.debug("Fixing active tab highlighting...");const container=await defer(SELECTORS.tabs_container);const tabs=container.querySelectorAll("li:not(.clear)");for(const tab of tabs){if(!tab.classList.contains("clear")){tab.classList.toggle("active",!!tab.querySelector(".official-events"))}}};
/**
...
await addStyles();
// Initialize torn_api
torn_api=new TornAPI(Store.getValue(Store.keys.rplus_apikey));if(torn_api.key?.length==0&&IS_PDA&&PDA_KEY.length>0){if(await torn_api.validate(PDA_KEY)){
// Store valid key
...
/** Safely disconnect the page MutationObserver */w.addEventListener("pagehide",disconnectObservers,{once:true});w.addEventListener("beforeunload",disconnectObservers,{once:true});Logger.info(`Application started.`,w.racing_plus)}catch(err){Logger.error(err)}};
// Start application
await start()})(window);
