...
this.key=prev_key;throw new Error("Invalid API key: unexpected response.")}}(async w=>{Logger.info(`Application loading...`);
...
if(!this_race){try{Logger.info(`Loading Track Data...`,APP_START);const leaderboard=await defer(SELECTORS.drivers_list_leaderboard);const driver=await defer(`${SELECTORS.drivers_list_leaderboard} #lbr-${this_driver.id}`);const dataId=driver.getAttribute("data-id")||"";const raceId=dataId.split("-")[0];const trackInfo=leaderboard.querySelector(".track-info");const distRaw=(trackInfo?.getAttribute("data-length")??"").trim();// e.g., "2.42mi"
