...
// ==/UserScript==
"use strict";
const MS=Object.freeze({second:1e3,minute:6e4,hour:36e5,day:864e5});const KMS_PER_MI=1.609344;const API_FETCH_TIMEOUT=10*MS.second;const DEFERRAL_TIMEOUT=15*MS.second;const SPEED_INTERVAL=MS.second;const CACHE_TTL=MS.hour;const SELECTORS=Object.freeze({links_container:"#racing-leaderboard-header-root div[class^='linksContainer']",main_container:"#racingMainContainer",main_banner:"#racingMainContainer .header-wrap div.banner",tabs_container:"#racingMainContainer .header-wrap ul.categories",content_container:"#racingAdditionalContainer",car_selected:"#racingupdates .car-selected",drivers_list:"#racingupdates .drivers-list",drivers_list_title:"#racingupdates .drivers-list div[class^='title']",drivers_list_leaderboard:"#racingupdates .drivers-list #leaderBoard"});
/**
...
page_observer?.disconnect()}catch(err){Logger.error(err)}};w.addEventListener("pagehide",disconnectObservers,{once:true});w.addEventListener("beforeunload",disconnectObservers,{once:true});Logger.info(`Application started.`,w.racing_plus)}catch(err){Logger.error(err)}};
// Start application
await start()})(window);