...
/**
   ...
   * @param {} tabs
   * @returns {Promise<void>}
   */const fixActiveTabHighlighting=async()=>{Logger.debug("Fixing active tab highlighting...");await defer(SELECTORS.tabs_container).children().querySelectorAll(":not(.clear)").forEach(c=>{c.classList.toggle("active",c.className==event.target.className)})};
/**
