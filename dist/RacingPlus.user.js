...
// }
}if(w.racing_plus.store.getValue(Store.keys.rplus_showskill)==="1"){if(!stats.querySelector(".skill")){stats.insertAdjacentHTML("afterBegin",'<div class="skill">RS: ?</div>')}if(torn_api.key){try{let user=await torn_api.request("user",`${driverId}/personalStats`,{stat:"racingskill"});if(user){let skill=stats.querySelector(".skill");skill.textContent=`RS: ${user.personalstats?.racing?.skill??"?"}`}}catch(err){console.log(`[TornPDA.Racing+]: ${err.error??err}`)}}}driverItem.classList.toggle("show",true)}//);
};
...
/** Initialize Torn API client with stored key or PDA key if applicable */logger.debug(`Initializing Torn API client...`,w.racing_plus);try{torn_api=new TornAPI(w.racing_plus.store.getValue(Store.keys.rplus_apikey));if(torn_api.key?.length==0&&IS_PDA&&PDA_KEY.length>0){await torn_api.validate(w.racing_plus.store,PDA_KEY);logger.debug("Valid API key.")}logger.info(`Torn API client nitialized.`,w.racing_plus.start)}catch(err){logger.error(err)}logger.debug(`Loading driver data...`,w.racing_plus.start);try{let scriptData=w.racing_plus.store.getValue(Store.keys.rplus_driver);if(!scriptData)scriptData=await defer("#torn-user").value;torn_driver=new TornDriver(JSON.parse(scriptData).id);torn_driver.load(w.racing_plus.store.getValue(Store.keys.rplus_driver));logger.info(`Driver data loaded.`,w.racing_plus.start)}catch(err){logger.error(`Failed to load driver data. ${err}`)}if(!IS_PDA){logger.debug("Fixing header banner...",w.racing_plus.start);const banner=await defer(SELECTORS.main_banner);const leftBanner=newElement("div",{className:"left-banner"});const rightBanner=newElement("div",{className:"right-banner"});const elements=Array.from(banner.children);elements.forEach(el=>{if(el.classList.contains("skill-desc")||el.classList.contains("skill")||el.classList.contains("lastgain")){if(el.classList.contains("skill")){torn_driver.updateSkill(el.textContent);el.textContent=String(torn_driver.skill)}leftBanner.appendChild(el)}else if(el.classList.contains("class-desc")||el.classList.contains("class-letter")){rightBanner.appendChild(el)}});banner.innerHTML="";banner.appendChild(leftBanner);banner.appendChild(rightBanner);logger.info("Header banner fixed.",w.racing_plus.start)}
// #################################################################################################################################################### //
/**
       * Start content for 'Official Events'
       */logger.debug(`Loading track data...`,w.racing_plus.start);try{const drivers_list=await defer(SELECTORS.drivers_list);const leaderboard=await deferChild(SELECTORS.drivers_list_leaderboard,"li[id^=lbr-]");
...
updateLeaderboard(leaderboard);logger.info(`Track data loaded.`,w.racing_plus.start)}catch(err){logger.error(`Failed to load track data. ${err}`)}
/**
       * End content for 'Official Events'
       */
// #################################################################################################################################################### //

logger.info(`Userscript started.`,w.racing_plus.start)}catch(err){logger.error(err)}};logger.info(`Userscript loaded. Starting...`,w.racing_plus.start);await start()})(window);