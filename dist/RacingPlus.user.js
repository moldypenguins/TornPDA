...
// ==/UserScript==
"use strict";
const API_KEY_LENGTH=16;const KMS_PER_MI=1.609344;const MS=Object.freeze({second:1e3,minute:6e4,hour:36e5,day:864e5});const API_FETCH_TIMEOUT=10*MS.second;const DEFERRAL_TIMEOUT=15*MS.second;const SPEED_INTERVAL=MS.second;const CACHE_TTL=MS.hour;const SELECTORS=Object.freeze({links_container:"#racing-leaderboard-header-root div[class^='linksContainer']",main_container:"#racingMainContainer",main_banner:"#racingMainContainer .header-wrap div.banner",tabs_container:"#racingMainContainer .header-wrap ul.categories",content_container:"#racingAdditionalContainer",car_selected:"#racingupdates .car-selected",drivers_list:"#racingupdates .drivers-list",drivers_list_title:"#racingupdates .drivers-list div[class^='title']",drivers_list_leaderboard:"#racingupdates .drivers-list #leaderBoard"});
/**
...
/**
   ...
   * @throws {Error}
   */async validate(key){if(!key||typeof key!=="string"||key.length!==API_KEY_LENGTH){throw new Error("Invalid API key: local validation.")}const prev_key=this.key;this.key=key;const data=await this.request("key","info",{timestamp:`${unixTimestamp()}`});if(data?.info?.access&&Number(data.info.access.level)>=ACCESS_LEVEL.Minimal){Logger.debug("Valid API key.");return true}this.key=prev_key;throw new Error("Invalid API key: unexpected response.")}}(async w=>{if(w.racing_plus)return;w.racing_plus=Date.now();
...
/**
     ...
     */
constructor(args={}){this.id=args.id??null;this.track=args.trackid?RACE_TRACKS[args.trackid]:null;this.title=args.title??"";this.distance=args.distance??null;this.laps=args.laps??null;this.status="unknown"}
/**
...
this.status=text.includes("starts: ")?"joined":"waiting";break}return this.status}
/**
...
/** @type {HTMLAnchorElement} */const apiStatus=await defer(".racing-plus-apikey-status");const apikey=torn_api.key??"";if(IS_PDA){apiInput.value=apikey;apiInput.disabled=true;apiInput.readOnly=true;apiStatus.textContent="Edit in TornPDA settings.";apiStatus.classList.toggle("show",true);apiSave.classList.toggle("show",false);apiReset.classList.toggle("show",false)}else{if(apikey.length>0){apiInput.value=apikey;apiInput.disabled=true;apiInput.readOnly=true;apiStatus.textContent="";apiStatus.classList.toggle("show",false);apiSave.classList.toggle("show",false);apiReset.classList.toggle("show",true)}else{apiInput.disabled=false;apiInput.readOnly=false;apiStatus.textContent="";apiStatus.classList.toggle("show",false);apiSave.classList.toggle("show",true);apiReset.classList.toggle("show",false)}apiSave.addEventListener("click",async ev=>{ev.preventDefault();const candidate=apiInput.value.trim();apiInput.classList.remove("valid","invalid");try{if(await torn_api.validate(candidate)){Logger.debug("Valid API key.");apiInput.classList.add("valid");torn_api.saveKey();apiInput.disabled=true;apiInput.readOnly=true;apiSave.classList.toggle("show",false);apiReset?.classList.toggle("show",true);if(apiStatus){apiStatus.textContent="";apiStatus.classList.toggle("show",false)}}}catch(err){Logger.warn(err);apiInput.classList.add("invalid");if(apiStatus){apiStatus.textContent=err.message??err;apiStatus.classList.toggle("show",true)}return false}});apiReset.addEventListener("click",ev=>{ev.preventDefault();if(!apiInput)return;apiInput.value="";apiInput.disabled=false;apiInput.readOnly=false;apiInput.classList.remove("valid","invalid");torn_api.deleteKey();apiSave?.classList.toggle("show",true);apiReset.classList.toggle("show",false);if(apiStatus){apiStatus.textContent="";apiStatus.classList.toggle("show",false)}})}w.document.querySelectorAll(".racing-plus-settings input[type=checkbox]").forEach(el=>{const key=Store.keys[el.id];if(!key)return;el.checked=Store.getValue(key)==="1";el.addEventListener("click",ev=>{const t=/** @type {HTMLInputElement} */ev.currentTarget;Store.setValue(key,t.checked?"1":"0");Logger.debug(`${el.id} saved ${t.checked?"on":"off"}.`)})})};
/**
   * Adds the settings buttons to the DOM
   * @param {HTMLElement} links_container
   * @returns {Promise<void>}
   */const addRacingPlusButton=async links_container=>{if(w.document.querySelector("#racing-plus-button"))return;const city_button=links_container.querySelector('[href="city.php"]');if(!city_button)return;const city_label=city_button.querySelector(`#${city_button.getAttribute("aria-labelledby")}`);const city_icon_wrap=city_button.querySelector(`:not([id])`);if(!city_label||!city_icon_wrap)return;const rplus_button=newElement("a",{role:"button",ariaLabelledBy:"racing-plus-link-label",id:"racing-plus-button",className:city_button.className,children:[newElement("span",{id:"racing-plus-button-icon",className:city_icon_wrap.className,width:16,height:16,innerHTML:'<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 15 14"><path d="m14.02,11.5c.65-1.17.99-2.48.99-3.82,0-2.03-.78-3.98-2.2-5.44-2.83-2.93-7.49-3.01-10.42-.18-.06.06-.12.12-.18.18C.78,3.7,0,5.66,0,7.69c0,1.36.35,2.69,1.02,3.88.36.64.82,1.22,1.35,1.73l.73.7,1.37-1.5-.73-.7c-.24-.23-.45-.47-.64-.74l1.22-.72-.64-1.14-1.22.72c-.6-1.42-.6-3.03,0-4.45l1.22.72.64-1.14-1.22-.72c.89-1.23,2.25-2.04,3.76-2.23v1.44h1.29v-1.44c1.51.19,2.87.99,3.76,2.23l-1.22.72.65,1.14,1.22-.72c.68,1.63.58,3.48-.28,5.02-.06.11-.12.21-.19.31l-1.14-.88.48,3.5,3.41-.49-1.15-.89c.12-.18.23-.35.33-.53Zm-6.51-4.97c-.64-.02-1.17.49-1.18,1.13s.49,1.17,1.13,1.18,1.17-.49,1.18-1.13c0,0,0-.01,0-.02l1.95-1.88-2.56.85c-.16-.09-.34-.13-.52-.13h0Z"/></svg>'}),newElement("span",{id:"racing-plus-button-label",className:city_label.className,innerText:"Racing+"})]});city_button.insertAdjacentElement("beforeBegin",rplus_button);rplus_button.addEventListener("click",ev=>{ev.preventDefault();Logger.debug("'rplus_button' clicked.");w.document.querySelector(".racing-plus-panel")?.classList.toggle("show")})};
/**
   * Fixes the top banner skill and class
   * @returns {Promise<void>}
   */const fixTopBanner=async()=>{const banner=await defer(SELECTORS.main_banner);const leftBanner=w.document.createElement("div");leftBanner.className="left-banner";const rightBanner=w.document.createElement("div");rightBanner.className="right-banner";const elements=Array.from(banner.children);elements.forEach(el=>{if(el.classList.contains("skill-desc")||el.classList.contains("skill")||el.classList.contains("lastgain")){if(el.classList.contains("skill")){this_driver.updateSkill(el.textContent);el.textContent=String(this_driver.skill)}leftBanner.appendChild(el)}else if(el.classList.contains("class-desc")||el.classList.contains("class-letter")){rightBanner.appendChild(el)}});banner.innerHTML="";banner.appendChild(leftBanner);banner.appendChild(rightBanner)};
/**
   * Fixes active tab highlighting for racing categories
   * @param {MouseEvent} event
   * @param {} tabs
   * @returns {Promise<void>}
   */const fixActiveTabHighlighting=async()=>{const container=await defer(SELECTORS.tabs_container);const tabs=container.querySelectorAll("li:not(.clear)");for(const tab of tabs){if(!tab.classList.contains("clear")){tab.classList.toggle("active",!!tab.querySelector(".official-events"))}}};
/**
   * start - Main entry point for the application.
   */const start=async()=>{try{Logger.info(`Application loaded. Starting...`,w.racing_plus);Logger.debug(`Injecting styles...`,w.racing_plus);await addStyles();Logger.debug(`Styles injected.`,w.racing_plus);torn_api=new TornAPI(Store.getValue(Store.keys.rplus_apikey));if(torn_api.key?.length==0&&IS_PDA&&PDA_KEY.length>0){if(await torn_api.validate(PDA_KEY)){Store.setValue(Store.keys.rplus_apikey,PDA_KEY)}}Logger.info(`Loading Driver Data...`,w.racing_plus);if(!this_driver){try{let scriptData=Store.getValue(Store.keys.rplus_driver);if(!scriptData){scriptData=await defer("#torn-user").value}this_driver=new TornDriver(JSON.parse(scriptData).id);this_driver.load()}catch(err){Logger.error(`Failed to load driver data. ${err}`)}}Logger.debug("Adding settings panel...",w.racing_plus);await addRacingPlusPanel();Logger.debug("Settings panel added.",w.racing_plus);const links_container=await defer(SELECTORS.links_container);Logger.debug("Adding settings button...",w.racing_plus);await addRacingPlusButton(links_container);Logger.debug("Settings button added.",w.racing_plus);const button_observer=new MutationObserver(async mutations=>{Logger.debug("Adding settings button...");await addRacingPlusButton(links_container);Logger.debug("Settings button added.")});button_observer.observe(links_container,{childList:true,subtree:true});Logger.debug("Initializing settings panel...",w.racing_plus);await initRacingPlusPanel(torn_api.key);Logger.debug("Settings panel initialized.",w.racing_plus);Logger.debug("Fixing top banner...",w.racing_plus);await fixTopBanner();Logger.debug("Top banner fixed.",w.racing_plus);Logger.debug("Updating driver records and cars...",w.racing_plus);
// await this_driver.updateRecords(); await this_driver.updateCars();
const results=await Promise.allSettled([this_driver.updateRecords(),this_driver.updateCars()]);if(!results.map(r=>r.status==="fulfilled")){Logger.error(results.filter(r=>r.status==="rejected").map(r=>`${r.status} - ${r.reason}`).join("\n"))}Logger.debug("Driver records and cars updated.",w.racing_plus);Logger.debug("Fixing active tab highlighting...",w.racing_plus);await fixActiveTabHighlighting();Logger.debug("Active tab highlighting fixed.",w.racing_plus);const drivers_list=await defer(SELECTORS.drivers_list);const list_title=await defer(SELECTORS.drivers_list_title);const leaderboard=await defer(SELECTORS.drivers_list_leaderboard);
/** @type {NodeListOf<ChildNode>} */const drivers=await new Promise(resolve=>{if(leaderboard.childNodes.length>0){resolve(leaderboard.childNodes)}else{const observer=new MutationObserver(()=>{if(leaderboard.childNodes.length>0){observer.disconnect();resolve(leaderboard.childNodes)}});observer.observe(leaderboard,{childList:true})}});if(!this_race){try{Logger.info(`Loading track data...`,w.racing_plus);const driver=Array.from(drivers).find(d=>d.id===`lbr-${this_driver.id}`);const dataId=driver.getAttribute("data-id");const raceId=dataId?.split("-")[0]??-1;const trackInfo=drivers_list.querySelector(".track-info");const track=Object.values(RACE_TRACKS).find(t=>t.name===trackInfo.getAttribute("title"));this_race=new TornRace({id:raceId,title:track.name,distance:track.distance,laps:track.laps});this_driver.load();Logger.info(`Track data loaded.`,w.racing_plus)}catch(err){Logger.error(`Failed to load track data. ${err}`)}}await this_race.updateLeaderboard(leaderboard.childNodes);Logger.debug(`Adding page observer...`,w.racing_plus);const content_container=await defer(SELECTORS.content_container);const page_observer=new MutationObserver(async mutations=>{for(const mutation of mutations){if(mutation.type==="characterData"||mutation.type==="childList"){const tNode=mutation.target;const el=tNode.nodeType===Node.ELEMENT_NODE?tNode:tNode.parentElement;if(el&&el.id==="infoSpot"){this_race?.updateStatus(el.textContent||"")}if(el&&el.id==="leaderBoard"){await(this_race?.updateLeaderboard(el.childNodes||[]))}}}});page_observer.observe(content_container,{characterData:true,childList:true,subtree:true});Logger.debug(`Page observer added.`,w.racing_plus);
/**
