...
// }
}if(w.racing_plus.store.getValue(Store.keys.rplus_showskill)==="1"){if(!stats.querySelector(".skill")){stats.insertAdjacentHTML("afterBegin",'<div class="skill">RS: ?</div>')}if(w.racing_plus.api.key){try{let user=await w.racing_plus.api.request("user",`${driverId}/personalStats`,{stat:"racingskill"});if(user){let skill=stats.querySelector(".skill");skill.textContent=`RS: ${user.personalstats?.racing?.skill??"?"}`}}catch(err){console.log(`[TornPDA.Racing+]: ${err.error??err}`)}}}driverItem.classList.toggle("show",true)}//);
};
...
updateLeaderboard(leaderboard);w.racing_plus.logger.info(`Track data loaded.`,w.racing_plus.start)}catch(err){w.racing_plus.logger.error(`Failed to load track data. ${err}`)}
/**
