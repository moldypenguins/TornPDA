...
Logger.debug(`Adding page observer...`,w.racing_plus);const content_container=await defer(SELECTORS.content_container);const page_observer=new MutationObserver(async mutations=>{Logger.debug(`Content Update -> ${mutations}.`);for(const mutation of mutations){if(mutation.type==="characterData"||mutation.type==="childList"){const tNode=mutation.target;const el=tNode.nodeType===Node.ELEMENT_NODE?tNode:tNode.parentElement;if(el&&el.id==="infoSpot"){this_race?.updateStatus(el.textContent||"")}if(el&&el.id==="leaderBoard"){await(this_race?.updateLeaderboard(el.childNodes||[]))}}}});page_observer.observe(content_container,{characterData:true,childList:true,subtree:true});Logger.debug(`Page observer added.`,w.racing_plus);
/**
