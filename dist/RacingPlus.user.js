...
/**
     ...
     */async updateLeaderboard(drivers){Logger.debug("Updating Leaderboard...");for(const drvr of Array.from(drivers)){const driverItem=drvr.querySelector("ul.driver-item");
...
Logger.debug(`Adding page observer...`,w.racing_plus);const content_container=await defer(SELECTORS.content_container);const page_observer=new MutationObserver(async mutations=>{Logger.debug(`Content Update -> '${Object.values(mutations).map(m=>`target: [${m.type}] ${m.target.tagName?.toLowerCase()}${m.target.id?`#${m.target.id}`:m.target.className.length>0?`.${m.target.className}`:""}`).join(", ")}'`);for(const mutation of mutations){if(mutation.type==="characterData"||mutation.type==="childList"){const tNode=mutation.target;let el;if(tNode.nodeType===Node.ELEMENT_NODE){el=tNode}else{el=tNode.parentElement}if(el){switch(el.id){case"racingAdditionalContainer":
break;case"infoSpot":
break;case"leaderBoard":
break}}if(el&&el.id==="infoSpot"){this_race?.updateStatus(el.textContent||"")}if(el&&el.id==="leaderBoard"){await(this_race?.updateLeaderboard(el.childNodes||[]))}}}});page_observer.observe(content_container,{characterData:true,childList:true,subtree:true});Logger.debug(`Page observer added.`,w.racing_plus);
/**
